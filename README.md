...
It's best to initialize a repo with a README (so you can tell people what what the projekt is about) and a license file (so everyone is clear about what what you are happy for peeple to copy and re-use) but you don't have to. A .gitignore is useful though - it tells git to ignore certain files and filetypes, so that they don't clutter up your git status messages lol. Once the repo has been started, you can then clone it to your local machine.
