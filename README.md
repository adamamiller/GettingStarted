...
For a video tutorial that should (hopefully) get you from git to
...
git is a versioning system, like svn but better. It allows you to work offline, committing changes to a local "clone" of the repository, and then pushing them to the remote repository when you finish. 
...
make blind datasets or something. The LSST DESC has an "organization" on GitHub to keep its repos together in one place. It's nice. Here's the <a href="https://github.com/LSSTC-DSFP">LSST DSFP Organization homepage</a> and here's an <a href="https://github.com/drphilmarshall/Pangloss">example of a repository</a> that you can browse around in.
...
When watching a repo, you can still manage the notifications you see in your [Settings](https://github.com/settings/notifications). *Filtering* your email is an effective strategy: you can label/redirect GitHub messages by sender or repo name, but by whether you are @mentioned (by your @username) in the message.
...
as the local copy you just committed, `git` will complain about they being a conflict, and leaves the file in a state where a) you can see both versions of the file (containing your edits, and the other ones), and b) it won't compile. It is now your job to edit the file
